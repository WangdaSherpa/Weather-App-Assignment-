...
document.querySelector(".weather_search").addEventListener('submit', e => {
    let search = document.querySelector(".weather_search_form");
...
// convert timezone and timestamp into current time
function convertTimeStamp(timestamp, timezone) {
    const convertTimezone = timezone / 3600; // convert seconds into hour
...
function getWeather() {
    const API_KEY = '91671fcce34ae02012047ec9983cdb5c';
    ...
    .then(data => {
        ...
        weather_temperature.innerHTML = `${data.main.temp.toFixed()}&#176&nbsp;${units === "imperial" ? "F": "C"}`;
